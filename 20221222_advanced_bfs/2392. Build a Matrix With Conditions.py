...
class Solution:
    ...
    def buildMatrix(self, k: int, rowConditions: List[List[int]], colConditions: List[List[int]]) -> List[List[int]]:
        ...
        for row in range(1, k+1):
            if row not in row_mapping: row_mapping[row] = []
        ...
        for node in list(row_mapping):
            if not row_visited[node - 1]: 
                if not self.dfs(node, row_mapping, row_visited, row_in_stack, row_stack): return []
                row_stack.append(node)

        col_mapping = defaultdict(set)
        self.build_graph(col_mapping, colConditions)
        for col in range(1, k+1):
            if col not in col_mapping: col_mapping[col] = []
        ...
        for node in list(col_mapping):
            if not col_visited[node - 1]:
                if not self.dfs(node, col_mapping, col_visited, col_in_stack, col_stack): return []
    ...
    def dfs(self, node, graph, visited, in_stack, stack) -> bool:
        ...
        for child in graph[node]:
            if not visited[child- 1]:
                if not self.dfs(child, graph, visited, in_stack, stack): return False
                stack.append(child)
            else:
                if in_stack[child - 1]: return False
