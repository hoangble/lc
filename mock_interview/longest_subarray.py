...
def longest_subarray_2d(prices: List[List], k: int) -> int:
    ...
    while left_corner_x < n and left_corner_y < m:
        while valid(left_corner_x + ans, left_corner_y + ans, m, n) and prefix_sum(con_sum_, left_corner_x, left_corner_y, left_corner_x + ans, left_corner_y + ans) <= k:
            print(left_corner_x, left_corner_y, ans, prefix_sum(con_sum_, left_corner_x, left_corner_y, left_corner_x + ans, left_corner_y + ans))
            ans += 1
        left_corner_y += 1
        if left_corner_y == n:
            left_corner_y = 0
    ...
    for i in range(m):
        ...
        for j in range(n):
            ...
            if i == 0:
                ...
                prefix_sum[i][j] = prices[i][j] + prefix_sum[prev_row][j] + prefix_sum[i][prev_col] - prefix_sum[prev_row][prev_col]
    return prefix_sum
...
def prefix_sum(prefix_sum: List[int], x0: int, y0: int, x1: int, y1: int) -> List[int]:
    ...
    return prefix_sum[x1][y1] - prefix_sum[x1][prev_y0] - prefix_sum[prev_x0][y1] + prefix_sum[prev_x0][prev_y0]
