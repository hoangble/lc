...
class Solution:
    ...
    def solveSudoku(self, board: List[List[str]]) -> None:
        ...
        for row in range(9):
            for col in range(9):
                if board[row][col] != ".":
                    num_ = board[row][col]
                    row_hash_map[row].add(
                        int(num_)
                    )  # = row_hash_map[row] | (1 << int(num_))
    ...
    def backtrack(self, i, j, board, row_hash_map, col_hash_map, box_hash_map):
        ...
        if board[i][j] != ".":
            ...
            if self.backtrack(
                i, j + 1, board, row_hash_map, col_hash_map, box_hash_map
            ):
                return True
            return False
        ...
        for x in range(1, 10):
            if (
                not x in row_hash_map[i]
                and not x in col_hash_map[j]
                and not x in box_hash_map[box_index]
            ):
